function [  ] = onlineRun(env, gS, T, path_WTS,index,U )
...
%for path_length=1:length(time_vector2)
    blend=[];time1=time_vector(index)-time_vector(index-1);
    [t,xq, Astar{path_length},Bstar{path_length}]=systevolution(path_WTS(path_length),path_WTS(path_length+1),A,T.ux_11+U(1),T.ux_12+U(2),T.ux_21+U(3),T.ux_22+U(4),T.uk_1+U(5),T.uk_2+U(6),x,y,time1,blend);
    stateend=size(xq,1);
    k=1;
