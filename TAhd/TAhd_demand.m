...
%Propostion x=1,3 and 2, yields transition from 1 to 2
A.trans{1, 2^1+2^2} = 2;
...
%Proposition 1 and not 2 and ap+1 yields transition from 1 to 3
...
%Proposition 1 and not ap+1 and not 2 yields transition from 1 to 4
A.trans{1, 2^1} = 4;
foo=[2.^(AP) 2.^(A.C+ap) 2^(c+ap+1)];
foo(ap+1)=[];
foo(2)=[];
...
%Proposition not ap+1 and not 1 nor 2 yields transition from 1 to 5
for x=3:ap
    A.trans{1, 2^(x)} = 5;
    foo=[2.^(AP) 2.^(A.C+ap) 2^(c+ap+1)];
    foo(ap+1)=[];
    ...
    for j = 1:length(foo)        
        bar = sum(nchoosek(foo,j),2);
        for foobar = 1:size(bar,1)
            A.trans{1, 2^(x) + bar(foobar)} = 5;
        end
    end
end
%Proposition not 1 and 2 yields transition from 1 to 6
A.trans{1, 2^2} = 6;
foo=[2.^(AP) 2.^(A.C+ap) 2^(c+ap+1)];
foo(2)=[];
foo(1)=[];
...
%Proposition not 1 yields transition from 2 to 6
for x=2:ap+1
    A.trans{2, 2^(x)} = 6;
    foo=[2.^(AP) 2.^(A.C+ap) 2^(c+ap+1)];
    foo(x)=[];
