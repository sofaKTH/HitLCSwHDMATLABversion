...
while pathFound==false && ~isempty(Spool)
    ...
    if any(ismember(final,q1))
        ...
        for q2=succ
            ...
            for x=X
                if Ix(q1,q2)==x
                    currTime=time(q1)+trans.in(q1,q2)+trans.out(q1,q2);
                    if currTime>=Xv(x)
                        violation=true;
                    end
                elseif Ixup(q1,q2)==x
            ...
            if violation==false
                ...
                if S(q1,1)==S(q2,1) && Ix(q2)~=0 && q1~=q2
                    ...
                    fprintf('updated trans time for %d -> %d \t new time= %f\n', q1, q2, trans.in(q1,q2));
                else
                    temptrans=trans.in(q1,q2)+trans.out(q1,q2);
                end
                currDC=dc(q1)+Ih(q1,1)*(trans.in(q1,q2)+trans.out(q1,q2));
                currDD=dd(q1)+Ih(q1,2)*trans.in(q1,q2)/2;
                currHD=c*currDC+(1-c)*currDD;
                if currHD<hd(q2)
                    %fprintf('we improved! old hd: %f new hd: %f\n', hd(q2), currHD);
                    hd(q2)=currHD;dc(q2)=currDC;dd(q2)=currDD;
                    parent(q2)=q1; time(q2)=time(q1)+trans.in(q1,q2)+trans.out(q1,q2);
                    Spool=[Spool q2]; %trans(q1,q2)=temptrans; not sure what this does...
                end
